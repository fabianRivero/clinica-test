...
The admin prospect list view MAY expose a badge or visual marker for `Prospecto.origen` in a future change. This change MUST NOT add a prospect-list origin column or badge; the topic is acknowledged here only so subsequent work does not contradict the `prospecto-origen` semantics.

#### Scenario: No prospect list badge in this change

- GIVEN the system after this change is deployed
- WHEN `/cms/prospectos` is rendered
- THEN no `origen` badge is displayed for any prospect row
