# Spec: Seed Client Baseline (Canonical)

> **Source of truth (2026-08-01)**: This file is the canonical spec for capability `seed-client-baseline`. It was assembled by merging two deltas in place: the original delta from `openspec/changes/add-seed-client-baseline/specs/seed-client-baseline/spec.md` and the delta from `openspec/changes/reform-database-seed-scripts/specs/seed-client-baseline/spec.md` (3 ADDED + 2 MODIFIED requirements). The original delta file in `add-seed-client-baseline/` has been updated in place to match this file and remains the working copy until that prior change is itself archived in a separate cycle.
>
> Future changes that modify the `seed-client-baseline` capability should author their delta against this canonical file.
