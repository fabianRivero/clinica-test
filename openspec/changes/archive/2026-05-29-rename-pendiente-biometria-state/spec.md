# Delta for Appointment States + Verification Cancel

## MODIFIED Requirements

### Requirement: Appointment State `REALIZADA_PENDIENTE_VERIFICACION`

The appointment state previously named `REALIZADA_PENDIENTE_BIOMETRIA` SHALL be renamed to `REALIZADA_PENDIENTE_VERIFICACION`. The display label SHALL change from "Realizada pendiente biometria" to "Realizada Pendiente de Verificación". This state MUST only be reachable from `PROGRAMADA`.
...
(Previously: enum value `REALIZADA_PENDIENTE_BIOMETRIA` with misleading "biometria" label)
