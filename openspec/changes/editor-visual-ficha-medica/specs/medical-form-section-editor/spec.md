...
Enable clinic admins to visually manage medical form sections (`FichaSeccion`) via the admin catalog API, with dual binding to `Sector` and/or `ProcEstetico`, and full CRUD operations including soft-delete and reordering.

## Requirements

### Requirement: REQ-1 — Section Creation with Sector Binding

The system SHALL allow creating a `FichaSeccion` with `sector` assigned and `proc_estetico` null. The API endpoint `POST /api/admin/catalogos/secciones-ficha/` MUST accept `{nombre, codigo, sector, orden, activo}` and persist the section scoped to the sector.

### Requirement: REQ-2 — Section Creation with ProcEstetico Binding

The system SHALL allow creating a `FichaSeccion` with `proc_estetico` assigned and `sector` null. The API MUST accept `{nombre, codigo, proc_estetico, orden, activo}` and persist the section scoped to the procedure.

### Requirement: REQ-3 — Section Creation with Dual Binding

The system SHALL allow creating a `FichaSeccion` with both `sector` and `proc_estetico` assigned simultaneously. Both bindings MUST be persisted.
...
The system SHALL support `PUT /api/admin/catalogos/secciones-ficha/<id>/` updating `nombre`, `codigo`, `sector`, `proc_estetico`, `orden`, and `activo`.
...
The system SHALL support changing `orden` via PATCH to modify display order. The API SHALL accept `{orden: <integer>}` and persist the new order value.
...
- WHEN `POST /api/admin/catalogos/secciones-ficha/` is called with `{nombre: "History", codigo: "HIST", sector: 1, orden: 1, activo: true}`
...
- WHEN the same POST request is made with `{nombre: "Consent", codigo: "CONS", proc_estetico: 2, orden: 1, activo: true}`
...
- WHEN POST is called with `{nombre: "Orphan", codigo: "ORPH", orden: 1, activo: true}`
...
- WHEN POST is called with `{nombre: "History 2", codigo: "HIST", proc_estetico: 2, orden: 2, activo: true}`
...
- GIVEN sections with `orden: 1, 2, 3`
- WHEN `PATCH /api/admin/catalogos/secciones-ficha/2/` with `{orden: 1}` is called
- THEN the response is HTTP 200 and the section's order is updated to `1`