...
export function SpecialistMessagesCreatePage() {
  ...
  return <SpecialistMessagingShell>
    <SectionCard eyebrow="Nueva ficha" title="Abrir comunicacion" description="Crea una ficha nueva con asunto y mensaje.">
      <form className="form-stack" onSubmit={(e) => { e.preventDefault(); const ok = window.confirm('¿Deseas enviar esta ficha ahora?'); if (!ok) return; setIsSending(true); void createTicket({ subject: form.subject, message: form.body }).then(() => { setForm(initialCompose); showNotification({ title: 'Ficha enviada', message: 'La ficha se envio correctamente.', tone: 'success' }) }).catch((err: unknown) => showNotification({ title: 'Error', message: err instanceof Error ? err.message : 'No se pudo crear la ficha.', tone: 'danger' })).finally(() => setIsSending(false)) }}>
...
export function SpecialistMessageDetailPage() {
  ...
  return <SpecialistMessagingShell>
    <SectionCard eyebrow='Detalle' title={ticket.subject} description={`Ficha ${ticket.status}`}>
      <div style={{ display: 'grid', gap: '0.85rem' }}>{messageItems.map(({ message: m, attachments }) => <article key={m.id} style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '0.85rem 1rem', background: 'var(--bg-card)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}><strong>{m.authorName}</strong><div style={{ display: 'flex', gap: '0.5rem' }}><small style={{ color: 'var(--c-neutral-600)' }}>{formatDateTime(m.createdAt)}</small><StatusBadge tone={m.status === 'RESPONDIDO' ? 'success' : 'warning'}>{m.status}</StatusBadge></div></div><p>{m.body}</p>{attachments.length ? <div style={{ marginTop: '0.75rem', display: 'grid', gap: '0.6rem' }}>{attachments.map((a) => <div key={`${m.id}-${a.url}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap', border: '1px solid var(--border)', borderRadius: '10px', padding: '0.55rem 0.65rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', minWidth: 0 }}>{a.isImage ? <button type='button' onClick={() => setPreviewImage(a)} style={{ border: '1px solid var(--border)', padding: 0, borderRadius: '8px', cursor: 'pointer', background: 'transparent' }}><img src={a.url} alt={a.name} style={{ width: '56px', height: '56px', objectFit: 'cover', display: 'block', borderRadius: '8px' }} /></button> : <span style={{ fontSize: '1.3rem' }}>{getFileTypeIcon(a.name, a.isImage)}</span>}<div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><span>{getFileTypeIcon(a.name, a.isImage)}</span><strong style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block', maxWidth: '320px' }}>{a.name}</strong></div></div><a className='button button--ghost button--compact' href={a.url} target='_blank' rel='noreferrer' download>Descargar</a></div>)}</div> : null}</article>)}</div>
