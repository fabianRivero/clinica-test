...
class CitasViewSet(viewsets.ViewSet):
    ...
    def pendiente_biometria(self, request, pk=None):
        ...
        Note: this endpoint is wired through admin_mark_appointment_pending_biometric
        in config/api_views.py. The action method below is kept for backward
        compatibility with any in-process calls but is not exposed via URLs
        (which route to the api_views function).
        """
        appointment = self._get_appointment(pk)
        if not appointment:
            return Response({"detail": "No encontramos la cita solicitada."}, status=404)
